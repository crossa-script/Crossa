...
The current foundation includes `import #filename.cra#`, `fun`, `re`, `var`, `print`, `model`, `config`, `Int`, `Long`, `Double`, `String`, `Bool`, `List<T>`, `@Sync`, `@Async`, `@AsyncAfter`, `#identifier` interpolation, and `CrossaRequest`. This list establishes integration points only; the language foundation remains authoritative for exact grammar and behavior.
