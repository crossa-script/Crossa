...
namespace crossa::compiler::generators::kotlin {
    ...
    KotlinGeneratedSource KotlinGenerator::generate(
        ...
        const string className = KotlinSourcePlanner::toPascalCase(
            program.getIdentity()
        );
        writer.beginBlock("public class " + identifierEscaper_.escape(className));
        ModelMap models;
    ...
    vector<KotlinGeneratedSource> KotlinGenerator::generateProject(
        const vector<const ir::Program*>& programs,
        const string& packageName,
        KotlinGenerationTarget target
    ) const {
        if (target != KotlinGenerationTarget::AndroidNative) {
            failUnsupported("project generation target");
        }
        if (!isValidPackageName(packageName)) {
            throw runtime_error("Kotlin config packageName is invalid.");
        }

        const KotlinProjectGenerationContext context(programs);
        const KotlinSourcePlanner planner;
        const vector<KotlinSourcePlan> plans = planner.plan(context, packageName);
        vector<KotlinGeneratedSource> sources;
        sources.reserve(plans.size());
        ModelMap models;
        for (const auto& entry : context.getModels()) {
            models.emplace(entry.first, entry.second);
        }

        for (const KotlinSourcePlan& plan : plans) {
            KotlinSourceWriter writer;
            writer.writeLine("// Generated by Crossa. Do not edit.");
            writer.writeLine();
            emitPackageDirective(plan.getPackageName(), writer);
            emitProjectImports(plan, context, packageName, writer);

            if (plan.getModel() != nullptr) {
                emitNativeModel(*plan.getModel(), writer);
            } else {
                const string className = filesystem::path(
                    plan.getRelativePath()
                ).stem().string();
                writer.beginBlock(
                    "public class " + identifierEscaper_.escape(className)
                );
                for (const ir::IrFunctionDeclaration* function :
                     plan.getFunctions()) {
                    writer.writeLine();
                    emitFunction(
                        *function,
                        models,
                        target,
                        plan.getSourceIdentity(),
                        writer
                    );
                }
                writer.endBlock();
            }
            sources.emplace_back(plan.getRelativePath(), writer.getSource());
        }
        return sources;
    }
    ...
    void KotlinGenerator::emitProjectImports(
        ...
        if (plan.getModel() != nullptr) {
            for (const ir::IrModelField& field : plan.getModel()->getFields()) {
                collectModelDependencies(field.getType(), modelNames);
            }
        }
        for (const ir::IrFunctionDeclaration* function : plan.getFunctions()) {
            for (const ir::IrParameter& parameter : function->getParameters()) {
                collectModelDependencies(parameter.getType(), modelNames);
            }
            collectModelDependencies(function->getReturnType(), modelNames);
        }
        ...
        for (const string& modelName : modelNames) {
            if (context.getModels().find(modelName) == context.getModels().end()) {
                throw runtime_error(
                    "Kotlin generation cannot resolve model symbol '" +
                    modelName + "'."
                );
            }
            const string qualifiedName =
                basePackageName + ".model." + modelName;
            if (plan.getPackageName() != basePackageName + ".model") {
                imports.insert(qualifiedName);
            }
        }

        map<string, string> namesBySimpleName;
        for (const string& qualifiedName : imports) {
            const size_t separator = qualifiedName.rfind('.');
            const string simpleName = separator == string::npos
                ? qualifiedName
                : qualifiedName.substr(separator + 1);
            const auto existing = namesBySimpleName.find(simpleName);
            if (existing != namesBySimpleName.end() &&
                existing->second != qualifiedName) {
                throw runtime_error(
                    "Kotlin generation found conflicting import name '" +
                    simpleName + "'."
                );
            }
            namesBySimpleName.emplace(simpleName, qualifiedName);
        }
        if (!imports.empty()) {
            for (const string& qualifiedName : imports) {
                writer.writeLine("import " + qualifiedName);
            }
            writer.writeLine();
        }
    }
    ...
    void KotlinGenerator::collectModelDependencies(
        const types::SemanticType& type,
        vector<string>& modelNames
    ) {
        if (type.getKind() == types::SemanticTypeKind::Model) {
            modelNames.push_back(type.getModelName());
            return;
        }
    ...
    string KotlinGenerator::getFileName(const ir::Program& program) const {
        return KotlinSourcePlanner::toPascalCase(program.getIdentity()) + ".kt";
    }
